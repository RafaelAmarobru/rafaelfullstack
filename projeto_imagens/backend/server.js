...
// constante para preparar o caminho para pasta 
const path = require("path"); 
 
// faz a montagem do caminho das pastas 
const pastaUploads = path.join(__dirname, "uploads"); 
fs.mkdirSync(pastaUploads, { recursive: true }); 
app.use("/uploads", express.static(pastaUploads)); 
 
 
// Configura como e onde o Multer salvará a imagem 
const storage = multer.diskStorage({ 
  ...
  destination: (req, file, cb) => { 
    ...
    // null = nenhum erro 
    // pastaUploads = pasta onde a imagem será salva 
    cb(null, pastaUploads); 
 
  },
   // Define o nome que a imagem receberá 
  filename: (req, file, cb) => { 
    ...
    // Gera um número baseado na data e hora atual 
      const data = Date.now(); 
 
    // Gera um número aleatório inteiro 
   
    const aleatorio = Math.round( 
      Math.random() * 1e9 
    ); 
 
    // Junta data + número aleatório + extensão 
    // Exemplo: 
    // 1760000000000-583746291.jpg 
    const novoNome = 
